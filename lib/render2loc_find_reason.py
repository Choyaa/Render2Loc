...
def loc_query(imgq_pth,mkpq,mkp3d,focal_length,max_error=5):
    height, width = cv2.imread(imgq_pth).shape[:2]
    cx = .5 * width
    cy = .5 * height
...
def main(config):
    ...
    all_query_name = list(np.array(all_K)[:,0])
    #5.读取render图像的pose文件
    t1 = time.time()
    loftr_thres = [0.2, 0.2, 0.5, 0.75]
    for iterative_num in range(1, num_iter+1):
        # r_pose_path = result_path + 'iterative' + str(iterative_num-1) +'/'+ 'query_estimated_pose.txt' #!##output_file  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
        print("==============", r_pose_path)
            ...
            for imgq_name in all_query_name:
                ...
                if len(render_candidate) == 0:
                    ...
                    if iterative_num == 1 or max_correct == 0:
                        ...
                        #设置内参
                        render_idx = all_query_name.index(imgq_name)   #!渲染的query没有内参
                        K_w2c = all_K[render_idx][1]   
                        ...
                        focal_length = all_K[query_idx][2]
                        # print('query_focal',focal_length)
                        ret = loc_query(imgq_pth,mkpq_final,Points_3D,focal_length,max_error=12)
                        if ret['success'] ==True:

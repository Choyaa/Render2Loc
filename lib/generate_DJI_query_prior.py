...
def Write_db_intrinscs(write_path, input_path):
    intrincs_ = [1920 ,1080, 1400.3421449, 1065.78947 ,960.0 ,540.0]
    ...
    with open(write_path,'w') as file_w:
        with open(input_path,'r') as file:
            for line in file:
                ...
                model_name = 'PINHOLE'
                intrincs_ = intrincs_
                data_line_str = str(name) + ' ' + model_name +' '+str(intrincs_[0])+' '+str(intrincs_[1])+' '+str(intrincs_[2])+' '+str(intrincs_[3])+' '+str(intrincs_[4])+' '+str(intrincs_[5])+' \n'
...
def write_ground_truth(write_path, input_path, sequence):
    ...
    with open(write_path, 'w') as file_w:
        for i in range(len(img_list)): #len(img_list)
            ...
            z = str(float(img_list[i]['Pose']['Center']['z'] ) )   #!+17 for prior
...
def main(config):
    ...
    for i in range(len(sequence_name)):
        prior_save_path = save_colmap_prior_path + sequence_name[i] + '_prior' + '.txt'
        constructed_image = write_ground_truth(prior_save_path, input_path_xml, i)

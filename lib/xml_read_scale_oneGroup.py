    ...
    for k,v in Photogroup_item.items():
        if k == 'Photo':
            for j in range((len(Photogroup_item[k]))):
                ...
                to_write = [Name, qvec[0],qvec[1],qvec[2],qvec[3],tx,ty,tz]
                line = " ".join([str(elem) for elem in to_write])
                count += 1
...
if __name__ == "__main__":
    path  = "/home/ubuntu/Documents/code/SensLoc/datasets/AirLoc_wild/jinxia_2304/"
    xml_dir = path+"query_img_0501_m3d - AT - export.xml"
